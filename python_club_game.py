...
class MeteorBase:
    ...
    def __init__(self):
        self.rectangle = pygame.Rect(120, 100, self.img.get_width(), self.img.get_height())
...
class Ball:
    ...
    def loop(self, number_of_row):
        for row in range(4):
            x = row * self.width
            y =  number_of_row * self.height
            img_coord = (x, y, self.width, self.height)
            self.image_coordinates.append(img_coord)
    ...
    for obj in all_game_objects:
        ...
        if isinstance(obj, Ball):
            for space_obj in all_game_objects:
                if isinstance(space_obj, MeteoritBig) or isinstance(space_obj, MeteoritMed):
